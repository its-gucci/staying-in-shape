...
parser = argparse.ArgumentParser(description='PyTorch PointNet Training')
...
parser.add_argument('-d', '--dataset', default='ShapeNet', 
                    choices=['ShapeNet', 'ModelNet40', 'MiniShapeNet'], help='Dataset to train on.')
...
# shape transformation arguments
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.rip:
        augmentation.append(shape_transforms.RandomRIP(3, 1000, 3, 0.85))
    if args.pcblur:
        augmentation.append(transforms.RandomApply([shape_transforms.SmoothingOp(k=24,temp=1)], p=0.5))
    if args.perturb:
        augmentation.append(shape_transforms.RandomPerturbationGaussian(0.02))
    if args.interp:
        augmentation.append(shape_transforms.RandomRbf(0, 0.02, 100))
    if args.rotation:
        augmentation.append(shape_transforms.rotate_all())
    if args.y_rotation:
    ...
    if args.dataset == 'ModelNet40':
        ...
        train_dataset = ShapeNetDataLoader(
            shapenet_path=args.data,
            #classes=None,
            #classes=['airplane'],
            npoints=args.npoints,
            transform=moco.loader.TwoCropsTransform(transforms.Compose(augmentation)),
        )
    elif args.dataset == 'MiniShapeNet':
        train_dataset = MiniShapeNetDataLoader(
            shapenet_path=args.data,
            npoints=args.npoints,
            transform=moco.loader.TwoCropsTransform(transforms.Compose(augmentation)),
        )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            }, is_best=False, filename='models/{}/checkpoint_{:04d}.pth.tar'.format(args.model_name, epoch))
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, _) in enumerate(train_loader):
        ...
        if args.gpu is not None:
            images[0] = images[0].cuda(args.gpu, non_blocking=True).squeeze()
            images[1] = images[1].cuda(args.gpu, non_blocking=True).squeeze()
